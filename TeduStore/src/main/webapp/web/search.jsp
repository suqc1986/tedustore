...
<body>
		...
		<form name="" action="" method="post">
			<section id="section">
				...
				<c:forEach items="${goodsList}" var="goods">
					<div style="width: 25%" class="lf box" id="d1" onclick="toItemInfo(${goods.id})">
						<div class="info">
						<div class="img pic">
							<img src="${pageContext.request.contextPath}${goods.image}" alt="" onclick="toItemInfo(${goods.id})" />
						</div>			
						<div class="describe">
							<p style="overflow: hidden;text-overflow: ellipsis;white-space: nowrap;"  onclick="toItemInfo(${goods.id})">${goods.title}</p>
	...
	<div style="text-align: right;height: 30px;font-size: 18px;width: 1000px;margin: 0 auto;">
		共${count}中商品，共${pages}页 | 
		<c:forEach var="i" begin="1" end="${pages}">
			...
			href="${pageContext.request.contextPath}/goods/showSearch.do?categoryId=${categoryId}&pageIndex=${i}">${i} </a>&nbsp;
		</c:forEach>
	</div>
	<!-- 尾部-->
<!-- 页面底部-->
...
<script type="text/javascript">
	...
	function toItemInfo(id) {
		if (id) {
			window.location.href="${pageContext.request.contextPath}/goods/goodsInfo.do?goodsId="+id+"&categoryId=${categoryId}";
		}else {
			alert("商品id不存在");
		}
	} 
