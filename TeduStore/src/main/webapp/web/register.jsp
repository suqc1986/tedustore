...
<body>
	...
	<header id="top">
		<div class="top">
			<img src="${pageContext.request.contextPath}/images/header/logo.png"
				alt="" /> <span>欢迎注册</span>
		</div>
	</header>
	<div class="parent">
				...
				<form id="form-register" method="post" action="">
					...
					<div class="form-group">
						<label for="email">邮箱：</label> <input autocomplete required
							...
							class="msg-default hidden" id="upwdspan">请输入合法的邮箱地址</span>
					</div>
					<div class="form-group">
	...
	<script>
		$('#bt-register').click(function() {
			...
			if (lengths == 5) {
				/*$("#form-register").serialize() 序列化from表单内部字段*/
				$.ajax({
					url : "reRegist.do",
					dataType : "json",
					type : "POST",
		...
		email.onblur = function() {
			if (this.validity.valueMissing) {
				...
				$.ajax({
					...
					success : function(data) {
						console.log(data);
						$("#upwdspan").html(data.message);
						if (data.state == 0) {
							$("#upwdspan").attr("class", "msg-error");
						} else {
							$("#upwdspan").attr("class", "msg-success");
						}
					},
					error : function(e) {
						console.log(e);
