        ...
        <div class="rs_content">
            ...
            <div class="rs_content_headPortrait">
	                ...
	                <!-- <span class="change_headPortrait same_click" data-toggle="modal" data-target="#avatar-modal" >更改头像</span> -->
	                	<form enctype="multipart/form-data"  style="display: inline-block; margin-left: 30px">
		                <input type="file" name="file" multiple="multiple" value="" id="iconPic" onchange="getImage()">
	                	</form>
	            </div>
            <!--用户名-->
         	<form id = "personInfo">
            ...
            <div class="rs_content_mail">
                ...
                <input class="ed_email" value="${user.email}"
                id ="email" name = "email" style="display: none;"/>
                <span class="rs_mail">${user.email }</span>
                <span class="same_click change_mail">更改邮箱</span>
            </div>
         </form>
            <!--保存按钮-->
            <div class="save" onclick = "btnUpdate()">
                保存更改
            </div>
        </div>
    </div>
	...
	function getImage(){
		...
		$.ajax({
			...
			success:function(data){
				console.log(data);
				if(data.state == 1){
					$("#icon").attr("src",getObjectURL(fileNode.files[0]));
				}
			}
		})
	}
